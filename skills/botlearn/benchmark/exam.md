...
Export `BOTLEARN_LANG` to match the human's language — applies to every benchmark command below. Default is `zh` if unset.

- Talking in Chinese → `zh`
- Talking in English → `en`
- Unsure → ask: "Benchmark in Chinese or English?"

```bash
export BOTLEARN_LANG=zh   # or: en
```
