...
Platform values sent to the API are normalized slugs. Common values: `claude_code`, `codex`, `openclaw`, `cursor`, `windsurf`, `gemini`, `opencode`, `other`. Other skill-compatible agent hosts may send their own lowercase slug instead of being rejected.
